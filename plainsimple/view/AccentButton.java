...
class AccentButton extends AnchorPane {
  ...
  private void initialize() {
      FXMLLoader fxmlLoader = new FXMLLoader(
              getClass().getResource("/fxml/AccentButton.fxml"));
  ...
  @Override public String toString() {
    if (accent >= 0x80 && accent <= 0xFF) {
      return Character.toString(EXTENDED[accent - 0x7F]);
    }
    return Character.toString(accent);
  }
  public int getAscii() {
    return (int) accent;
  }
  public String insertAccent(String selection, String text, int caret_location) {
    if(selection.equals("") ) { /* simply insert the accent */
      if(caret_location == text.length() - 1) { /* added to end of String */
        return text + toString(); /* append accent */
      } else {
        return text.substring(0,
                              caret_location) + toString() + text.substring(caret_location);
      }
    } else if(text.endsWith(selection)) {
      return text.substring(0, caret_location) + toString();
    } else { /* replace selected text with accent */
        return text.substring(0,
                              caret_location) + toString() + text.substring(caret_location + selection.length());
    }
  }
