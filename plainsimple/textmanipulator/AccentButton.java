...
public class AccentButton {
    ...
    public int getAscii() {return (int) accent;}
    public String insertAccent(String selection, String text, int location) {
        if(selection == "") { /* simply insert the accent */
            return text.substring(0, location) + getAccentString() + text.substring(location + 1);
        } else { /* replace selected text with accent */
            return text.substring(0, location) + getAccentString() + text.substring(location + selection.length());
        }
    }
